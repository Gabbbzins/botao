...
function webHandler(event){
    ...
    let buttonCenter = {
            x: button.left + $('#no') .width() / 2,
            y: button.top + $('#no') . height() / 2
    }
    ...
    if (distance < 80){
        var angle = calculateAngle (event, buttonCenter, distance)
           ...
           if(newLeft + buttonWidth > buttonWidth) newLeft = windowWidth - buttonWidth
           if(newtop + buttonHeight> buttonHeight) newtop = windowHeight - buttonHeight
